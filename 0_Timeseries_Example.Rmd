...
jsFiltered <- jsFilterd %>% filter(secchidepth < 5) %>%
  group_by(date) %>%
...
## look at if there's a seasonal signal
jsFiltered %>%
  mutate(month = month(date, label = T)) %>%
  ggplot(aes(x = month, y = secchidepth)) +
  geom_boxplot()
# Except for February it totally looks like there is


### Total bonus, do a seasonal decomposition.  Here I use an approach called STL, Johnathon would give you big bonus points if you did a similar thing with Forier Transfroms.  Both do a similar thing where they extract the seasonal component and the overall trend

## Also, take all this with a grain of salt, there's a lot of missing data here

## First summarise everything to a monthly value
...
## Pull out the summary values from the resulting stl object
ts.summary <- check$data %>% select(raw, seasonal, trend, remainder) %>%
  mutate(timestep = row_number()) %>%
  pivot_longer(-timestep, values_to = 'Secchi', names_to = 'Component')
  
## Plot up the seasonal and trend parts.
ggplot(ts.summary %>% filter(Component %in% c('trend', 'seasonal')), aes(x = timestep, y = Secchi)) +
  geom_line() +
